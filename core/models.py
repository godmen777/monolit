...
class Template(models.Model):
	name = models.CharField(u'Название',
							max_length=50,
							unique=False)
	template = models.CharField(u'Шаблон',
							help_text=u'путь до шаблона, например "core/services_set.html"',
							max_length=200,
							unique=False)
...
class Service(MPTTModel):
	...
	image_set_crop   = ImageRatioField('image',
																			'470x526', 
																			verbose_name=u'Обрезка для услуги 470x526')
	image_list_crop  = ImageRatioField('image',
																			'750x365', 
																			verbose_name=u'Обрезка для списка услуг 750x365')
	bg_image         = models.ImageField(upload_to="services",
																			verbose_name=u'Фоновое изображение')
	preview_text     = models.TextField(verbose_name=u"Превью")
...
class Post(models.Model):
	service          = models.ForeignKey(Service, 
											verbose_name=u"Связать с услугой")
	name             = models.CharField(u'Название',
											max_length=50,
											unique=False)
	...
	image_list_crop  = ImageRatioField('image',
																			'750x365', 
																			verbose_name=u'Обрезка для списка статей 750x365')
	preview_text     = models.TextField(verbose_name=u"Превью")
	text             = RichTextUploadingField()
	meta_title       = models.CharField(verbose_name=u'Мета title',
											max_length=80,
