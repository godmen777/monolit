...
class Project(models.Model):
	...
	for_sale = models.BooleanField(default=False, verbose_name=u"на продажу")
...
class ProjectImage(models.Model):
	...
	cropping = ImageRatioField('image', 
							'500x320', 
							verbose_name=u'Обрезка для проекта')

	cropping_250x375 = ImageRatioField('image', 
							'250x375', 
							verbose_name=u'Обрезка для проекта')

	cropping_720x400 = ImageRatioField('image', 
							'720x400', 
							verbose_name=u'Обрезка для катрочки проекта')
